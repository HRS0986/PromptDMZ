...
- **Process:** For each adapter, wrap the prompt in that adapter's instruction template using Gemma chat format, ending exactly at the model turn **with no answer text**. Tokenize the three strings as one padded batch (left-padding for causal LM; `max_length=2048`, truncation documented — NOTE training used `max_length=512`, so document the mismatch or match it).
- **VERIFIED FORMAT (230,992/230,992 deployed rows).** The turn separator is `"\n" + 8 spaces`, not `\n`, and a literal `f'` (a leaked Python f-string prefix) sits between the turns. Both are part of the training distribution and must be reproduced verbatim:
  ```
  <bos><start_of_turn>user\n        {INSTRUCTION}User Prompt:{prompt}Respond with exactly one word: INJECTION or BENIGN<end_of_turn>\n        f'<start_of_turn>model\n        {ANSWER}<end_of_turn>
  ```
  Scoring inputs end at `<start_of_turn>model\n        ` so the next predicted token is the label word, not whitespace. `<bos>` is omitted (training stripped it; the tokenizer adds it).
...
- **CORRECTED — there is NO template variation.** This was assumed from `EDA.ipynb`'s markdown, but that notebook is **stale**: it pushed to `slm-shield-ds-*` repos (50/50 class balance, different formatter), while the adapters were trained on `fyp-slm-*` (75/25). The deployed data has **exactly one** template variant per adapter, confirmed across all 230,992 rows. Consequence: the two scoring modes below produce byte-identical strings, so cross-adapter scoring is exact rather than approximate (`assert_modes_agree()` is the regression test). The modes are retained as separate entry points in case a future dataset does introduce variation:
...
- **Fallback (must be implemented):** if `adapter_names` is unsupported/incorrect on the pinned stack, run three sequential single-row forward passes with `set_adapter` — no early exit. The architecture is unchanged; only latency differs. Verification requirement: batched and sequential must agree on every **decision** (argmax identical) on ≥200 prompts, with median/p95/max |Δp| reported (P1.2).
- **MEASURED (2026-07-26, T4) — batched and sequential are NOT bitwise equal, and cannot be.** fp16/NF4 matmul reduction order depends on batch shape, so a row inside a padded 3-row batch differs from the same row scored alone. `diagnose_divergence` attributes it: determinism max |Δd| = **0.0** (identical inputs → identical outputs), batch_vs_single = 0.1875, adapter_mixing = 0.15625 — both fp16-grid values (~6 ULPs at logit magnitude ~32), and both the same effect, since PEFT sub-batches per adapter. No defect exists to fix. Consequence, and the reason determinism = 0.0 matters: **fixing the batch shape makes scoring exactly reproducible**, so all scoring runs at `batch_size=1` (the `[3, L]` shape above), which is also what C10's server does per request — the evaluated configuration is the deployed configuration. Report the batch-shape sensitivity as a limitation in §4; it is a genuine finding about serving multiple adapters on a quantized backbone, not an implementation wart.
